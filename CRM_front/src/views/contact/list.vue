  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" align="center" label="客户名称">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.cust.name}}</span>
          <!-- <el-tag>金桥文苑</el-tag> -->
        </template>
      </el-table-column>
      ...
      <el-table-column width="110px" align="center" label="部门名称">
        <template slot-scope="scope">
          <span>{{scope.row.dept}}</span>
        </template>
      </el-table-column>
      <el-table-column width="110px" align="center" label="客户手机">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      axios.post('api/contacts/getAll').then(response => {
